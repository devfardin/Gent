...
class WhatsApp
{
    ...
    {
        add_action('wp_footer', [$this, 'whatsapp_script']);
    }

    public function whatsapp_script()
    {
        ...
        if (!$whatsapp_number) {
            return;
        } else if ($show_on_desktop == 0) {
            return;
        }
        ?>
        <a class="wa-float" href="https://wa.me/<?php echo esc_attr($whatsapp_number); ?>?text=<?php echo $message; ?>"
            target="_blank" rel="nofollow noreferrer" aria-label="Chat on WhatsApp">
            <div class="wa-float__label"><?php echo esc_html($btn); ?></div>
